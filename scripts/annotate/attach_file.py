...
def main(conn, args):
  ...
  if args.name:
    tmp = list(conn.getObjects(target_type, attributes={"name": target_id}))
  else:
  ...
  if args.upload:
    fo = conn.createOriginalFileFromLocalFile(path.resolve(), mimetype=args.mimetype, ns=args.namespace)
    fa.setFile(fo._obj)
  else:
...
if __name__ == '__main__':
  ...
  parser.add_argument("-m", "--mimetype", default=MIMETYPE,
    help="Mimetype (default: text/csv)")
  parser.add_argument("-ns", "--namespace", default=NAMESPACE,
  ...
  parser.add_argument("-n", "--name", action="store_true",
    help="Use target name (e.g. Project:idr0111-lee-cellmigration/experimentA) (default: id)")
